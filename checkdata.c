...
_________________________________________________________________________
    ...
    Used to check input and intermediate data via writing file in struct sound format. 
*/
...
int checkdata(struct sound *data)
{
  ...
  printf("In checkdata; output file name is datacheck: data->nht = %4d\n\n", data->nht); 
  ...
  fprintf(fpout,"    hgt    prs     tmp     u       v       spd     dir\n\n");

  for(i=0;i<data->nht;i++)  
    {
      fprintf(fpout,"%7.0f %7.2f %7.2f %7.2f %7.2f %7.2f %7.2f\n",
          data->level[i].hgt, data->level[i].prs, data->level[i].tmp,
