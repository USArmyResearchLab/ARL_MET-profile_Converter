...
int clean_data(struct sound *data, float min_height)
{
  ...
  */
   ...
   /* set check for upps or raob output = this program's input */
   check.prs_min=2.0;
   check.prs_max=1090.0;
   check.hgt_min=min_height; 
   check.hgt_max=MAXHEIGHT;
   check.tmp_min=-120; /*degrees C*/
   check.tmp_max=80.0;
   check.dew_min=-125;  /*dewpoint (noaa input uses dewpoint vs. relative humidity)*/ 
   check.dew_max=80.0;
   check.spd_min=0;
   check.spd_max=220.0;
   check.dir_min=0;
 ...
 for(i=0;i<data->nht;i++) 
  {
   if(data->level[i].hgt != ERROR && (data->level[i].hgt < check.hgt_min ||
  ...
  else 
    {
     ...
     if(data->level[i].dew != ERROR && (data->level[i].dew < check.dew_min ||
	 data->level[i].dew > check.dew_max))
            data->level[i].dew = ERROR;
