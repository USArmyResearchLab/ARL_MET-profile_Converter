...
This function prints height based layer type output to the appropriate file.  It writes an ouput sounding for 
a user defined height or pressure level profile.  Input data are taken from the University of Wyoming web site, 
NOAA archive, or other source of upper air files. If wind speed is in m/s, "uncomment" the appropriate lines 
near the end. 
*/
...
int writeusrmsg(struct sound *user, char *outpath)
{
  ...
  /* Open output file.  ************************************/
   ...
   /*   USER DEFINED layer output output ***********************/
    ...
    fprintf(fuserout,"Layer   Height  Wind Direction  Wind Speed   Virt Temp  Pressure  Temperature\n");
