...
This function prints pressure based layer output to the appropriate file.  It writes an output sounding for 
a user defined pressure level structure. The data lines in the output file have pressure as the first variable.  
Input data are taken from the University of Wyoming web site, NOAA archive, or other source of upper air files.  
If wind speed is in m/s, "uncomment" the appropriate lines near the end. 
*/
...
int writeusrmsg(struct sound *user, char *outpath)
{
  ...
  /* Open output file.  ************************************/
     ...
     if(!(fuserout = fopen(outfile,"w")))
      {
        fprintf(stderr,"Unable to open user msg output data file.\n");
        exit(1);
      }
